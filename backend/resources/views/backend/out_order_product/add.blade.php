...
@section('form_content')
    <form class="form-horizontal" method="POST" action="<?=route('order_product.store',['order_code' => $order_code,'order_type' => $order_type]);?>">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <select name="product_id" class="form-control select2" id="formProductID" required autofocus>
                    ...
                    <?php
                        ...
                        {
                            $data_json[$product->id] = $product->qty_disp;
...
@section('js_footer')
    <script>
        ...
        {
            var info_products = JSON.parse('<?=json_encode($data_json);?>');

            $('#formProductID').change(function ()
            {
                $('#formQuantity').attr('max',parseInt(info_products[$(this).val()]));
            });
        });
    </script>
@endsection
