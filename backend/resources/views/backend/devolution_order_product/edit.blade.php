...
@section('form_content')
    <form class="form-horizontal" method="POST" action="<?=route('order_product.update',['order_code' => $order_code,'order_type' => $order_type, 'id' => $order_product->pivot->id]);?>">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="number" min="1" name="quantity" class="form-control" id="formQuantity" value="<?=$order_product->pivot->quantity;?>" max="<?=$order_product->qty_disp;?>" placeholder="¿Cantidad?" required autocomplete="off">
            </div>
        </div>
