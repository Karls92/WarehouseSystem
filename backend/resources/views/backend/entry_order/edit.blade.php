<?php /* @var App\Models\Order $entry_order */ ?>
@extends('backend.general.form_basic')

@section('form_content')
    <form class="form-horizontal" method="POST" action="<?=route('entry_order.update',['slug' => $entry_order->slug]);?>">
            ...
            <div class="col-sm-5">
                <select name="client_id" class="form-control select2" id="formClientID" required autofocus>
                    ...
                    <?php
                        ...
                        {
                            ?>
                            <option value="<?=$client->id;?>" <?=($client->id == $entry_order->client_id) ? 'selected' : '';?>><?=$client->name;?></option>
                            <?php
                        }
                    ?>
                </select>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="text" name="received_by" class="form-control" id="formReceivedBy" value="<?=$entry_order->received_by;?>" placeholder="¿Recibido Por?" required autocomplete="off">
            </div>
        </div>
        <div class="form-group">
            <label for="#formDeliveredBy" class="col-sm-2 control-label">Entregado Por</label>
            <div class="col-sm-5">
                <input type="text" name="delivered_by" class="form-control" id="formDeliveredBy" value="<?=$entry_order->delivered_by;?>" placeholder="¿Entregado Por?" required autocomplete="off">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="input-group col-sm-5 custom_datepicker date" id="datepicker">
                <input type="text" class="form-control" readonly="readonly" id="formDate" name="date" value="<?=custom_date_format($entry_order->date);?>" required>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <textarea name="description" class="form-control" id="formDescription" placeholder="¿Observaciones?" rows="5"><?=$entry_order->description;?></textarea>
            </div>
        </div>
