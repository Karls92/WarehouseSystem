...
@section('form_content')
    <form class="form-horizontal" method="POST" action="<?=route('devolution_order.store');?>">
        ...
        <div class="form-group">
            <label for="#formOrderID" class="col-sm-2 control-label">out order</label>
            <div class="col-sm-5">
                <select name="out_order_id" class="form-control select2" id="formOrderID" required>
                    <option value="">--- SELECT ---</option>
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <input type="text" name="received_by" class="form-control" id="formReceivedBy" value="<?=(!is_null(old('received_by')) && strlen(old('received_by')) > 0) ? old('received_by') : Auth::user()->full_name;?>" placeholder="¿Received by?" required autocomplete="off">
            </div>
        </div>
        <div class="form-group">
            <label for="#formDeliveredBy" class="col-sm-2 control-label">Delivered by</label>
            <div class="col-sm-5">
                <input type="text" name="delivered_by" class="form-control" id="formDeliveredBy" value="<?=old('delivered_by');?>" placeholder="¿Delivered by?" required autocomplete="off">
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <textarea name="description" class="form-control" id="formDescription" placeholder="¿Observations?" rows="5"><?=old('description');?></textarea>
            </div>
        </div>
        <div class="form-group" id="formSubmit">
            <div class="col-sm-offset-2 col-sm-5">
                <hr>
                <button type="submit" class="btn btn-success pull-right"><span class="glyphicon glyphicon-plus-sign"></span> Add order</button>
            </div>
        </div>
    </form>
@endsection
